...
static void mxs_auart_tx_chars(struct mxs_auart_port *s)
{
	...
	if (test_bit(MXS_AUART_RS485, &s->flags) &&
	    (s->port.x_char ||
	     (!kfifo_is_empty(&tport->xmit_fifo) && !uart_tx_stopped(&s->port))))
	...
	if (test_bit(MXS_AUART_RS485, &s->flags)) {
		...
		if (!kfifo_is_empty(&tport->xmit_fifo))
			timeout =
				ns_to_ktime(jiffies_to_nsecs(uart_fifo_timeout(&s->port)));
				 ...
				 const struct ktermios *old)
{
	...
	if (!test_bit(MXS_AUART_RS485, &s->flags)) {
		...
		if (cflag & CRTSCTS) {
			 ...
			 * in mx28.
			 */
			if (is_imx28_auart(s) && test_bit(MXS_AUART_RTSCTS,
							  &s->flags)) {
...
static int mxs_auart_probe(struct platform_device *pdev)
{
	...
	hrtimer_setup(&s->rs485_finish_timer, mxs_auart_tx_rs485_finish,
